...
public class JmsOperations {
	...
	@Summary("Indicate which JMS config should be associated with this healtcheck.")
	@Placement(order = 2)
	...
	public Healtcheck getJmsHealthCheck() {

		LOGGER.debug("START: JMS HEALTCHECK parameters:");


		String destQueue = this.queueDestination;

		LOGGER.debug("JMS HEALTCHECK on jms: with config-ref: "+configRef+" to queue:"+destQueue);
		...
		try {
			...
			OperationParameters parameters = DefaultOperationParameters.builder().configName(configRef)
					...
					.build();
			LOGGER.debug("JMS HEALTCHECK parameters:"+parameters);
		...
		return new Healtcheck(serviceName, type, status,(Integer) Math.toIntExact(elapsedTime),errorString);
