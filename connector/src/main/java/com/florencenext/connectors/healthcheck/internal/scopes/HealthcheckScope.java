...
public class HealthcheckScope {
    ...
    public void hcScope(Chain operations,
                        CompletionCallback<Healtcheck, Void> callback) {

        LOGGER.debug("Invoking child operations");


        Healtcheck outputHc = new Healtcheck(
                serviceName,
                serviceType,
        ...
        operations.process(
                ...
                (error, previous) -> {
                    ...
                    callback.success(Result.<Healtcheck,Void>builder().output(outputHc).build());
                });
    }
}
