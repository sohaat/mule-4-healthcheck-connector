...
public class InternalOperations {
    ...
    private final String DEFAULT_EXPRESSION = "output application/json import * from modules::HealtcheckConnectorModule --- payload as HealthCheck";
    ...
    public List<Healtcheck> getHealthCheckFromInternalServices(@Config HealtcheckConnectorConfig c) {

        LOGGER.debug("START: internalServices healtcheck");

        String healthcheckPath = c.getHealtcheckPath();
        List<InternalServiceConfig> internalServicesUris= c.getInternalServicesConfigs();

        List<Healtcheck> healthCheckListResult = new ArrayList<Healtcheck>();
            ...
            try {
                ...
                elapsedTime = System.currentTimeMillis() - startTime;
                    ...
                    if(attributes.get().getStatusCode() == 200) {
                        ...
                        TypedValue transformedHealtcheck = null;
                        DataType dataType = DataType.builder()
                                .type(Healtcheck.class)
                        ...
                        try {
                            transformedHealtcheck = expressionManager.evaluate(dwToRun,bindingContext);
                        } catch (RuntimeException e ) {
                            e.printStackTrace();
                        }

                        LOGGER.debug("Output after apply internal dataweave: "+transformedHealtcheck.getValue());
                        ObjectMapper mapper = new ObjectMapper();
                        JsonNode actualObj = mapper.readTree(transformedHealtcheck.getValue().toString());
                        Healtcheck serviceHc = mapper.convertValue(actualObj, Healtcheck.class);
                        ...
                        Healtcheck serviceInError = new Healtcheck(internalServiceName,
                                ServiceType.HTTP,
                ...
                Healtcheck serviceInError = new Healtcheck(
                        internalServiceName,
                        ServiceType.HTTP,
                        ServiceStatus.UNHEALTHY,
                        (Integer) Math.toIntExact(elapsedTime),
