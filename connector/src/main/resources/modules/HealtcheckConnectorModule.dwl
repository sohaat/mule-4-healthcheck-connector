...
fun formattedHealtcheck(h:HealthCheck) : HealthCheck = {
    name: h.name,
    "type": h."type",
    status: h.status,
    time: h.time,
    error: h.error,
    dependencies: (h.dependencies default []) map formattedHealtcheck($),
    dependenciesErrors: h.dependenciesErrors map {name: $.name,error: $.error}
}

fun formattedHealtcheck(h:HealthCheck,formatProperties:FormatProperties) : HealthCheck = {
    name: h.name,
    "type": h."type",
    status: h.status,
    (time: h.time) if(!isEmpty(h.time)),
    (error: h.error) if(!isEmpty(h.error)),
    dependencies: (h.dependencies default []) map formattedHealtcheck($,formatProperties),
